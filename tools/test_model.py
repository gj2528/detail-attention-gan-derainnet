...
from attentive_gan_model import derain_drop_pspnet
from attentive_gan_model import derain_cnn_psp
...
def init_args():
    ...
    parser.add_argument('--image_path', type=str, help='The input image path',default='../data/test_b/data/98_rain.jpg')
    ...
    # parser.add_argument('--weights_path', type=str, help='The model weights path',default='./model/derain_gan/derain_gan_2019-11-12-09-22-36.ckpt-2000')

    parser.add_argument('--label_path', type=str, default='../data/test_b/gt/98_clean.jpg', help='The label image path')
    # parser.add_argument('--output_path', type=str, default='../data/test_data/test_output/', help='The output image path')
...
def test_model(image_path, weights_path, label_path=None):
    ...
    with sess.as_default():
        ...
        # 保存并可视化结果
        cv2.imwrite(args.output_path + 'src_img.png', image_vis)
        cv2.imwrite(args.output_path + 'derain_ret.png', output_image)
        ...
        plt.savefig(args.output_path + 'atte_map_1.png')
        plt.figure('atte_map_2')
        plt.imshow(atte_maps[1][0, :, :, 0], cmap='jet')
        plt.savefig(args.output_path + 'atte_map_2.png')
        plt.figure('atte_map_3')
        plt.imshow(atte_maps[2][0, :, :, 0], cmap='jet')
        plt.savefig(args.output_path + 'atte_map_3.png')
        plt.figure('atte_map_4')
        plt.imshow(atte_maps[3][0, :, :, 0], cmap='jet')
        plt.savefig(args.output_path + 'atte_map_4.png')
        plt.show()
