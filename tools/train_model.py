...
def init_args():
    ...
    parser.add_argument('--dataset_dir', type=str, help='The dataset dir',default='../data')
...
def save_map(atte_maps, step):
    for num in range(4):
        plt.figure(str(step) + 'atte_map_' + str(num))
        plt.imshow(atte_maps[num][0, :, :, 0], cmap='jet')
        plt.savefig("./attentive_img/" + str(step) + 'atte_map_' + str(num) + '.png')
...
def train_model(dataset_dir, weights_path=None):
    ...
    with tf.device('/gpu:1'):
        ...
        output, attention_maps, ret = derain_net.inference(input_tensor=train_input_tensor, name='derain_net', reuse=True)
        ...
        with sess.as_default():
            ...
            for epoch in range(train_epochs):
                ...
                if epoch % 1 == 0:
                    output_image, atte_maps, ret_ = sess.run(
                        [output, attention_maps, ret])
                    ...
                    cv2.imwrite('./output_img/{}derain_ret.png'.format(epoch), output_image)

                    save_map(atte_maps, epoch)

                    from scipy import misc
                    for x in range(1,4):
                        X = ret_['skip_'+str(x)][0]
                        misc.imsave('test_imgs/' + str(epoch) + 'skip_'+ str(x) + '.png', X)
