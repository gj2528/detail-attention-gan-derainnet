...
def init_args():
    ...
    parser.add_argument('--dataset_dir', type=str, help='The source nsfw data dir path',default='../data/train_data')
    parser.add_argument('--tfrecords_dir', type=str, help='The dir path to save converted tfrecords',default='../data/tfrecords')
...
class DerainDataFeeder(object):
    ...
    def inputs(self, batch_size, num_epochs):
        ...
        with tf.name_scope('input_tensor'):
            ...
            # TFRecordDataset打开一个二进制文件，一次读取一个记录。
            # tfrecords_file_paths也可以是文件名列表，将按顺序读取。
            ...
            # 随机播放转换使用有限大小的缓冲区对内存中的元素进行随机播放。
            # 该参数是缓冲区中元素的数量。 要完全均匀地进行混洗，
            # 请将参数设置为与数据集中的元素数相同。
            if self._dataset_flags != 'test':
                dataset = dataset.shuffle(buffer_size=1000)
                # repeat num epochs 重复数个纪元
