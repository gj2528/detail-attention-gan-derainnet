#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time    : 19-11-11 下午3:45
# @Author  : gj2528
# @Site    : https://github.com/
# @File    : att_gan_cnn_psp.py
# @IDE: PyCharm
"""
...
class GenerativeNet(cnn_basenet.CNNBaseModel):
    ...
    def pspnet_block(self,input,pool_k_h,pool_k_w,pool_s_h,pool_s_w,shape,name,reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            avg_pool = tf.nn.avg_pool(input, ksize=[1, pool_k_h, pool_k_w, 1],
                                       strides=[1, pool_s_h, pool_s_w, 1], padding='VALID',
                                       name='avg_pool')
            conv = tf.layers.conv2d(avg_pool, filters=256, kernel_size=[1, 1],
                                    strides=[1, 1], padding='VALID', use_bias=False,name='conv')

            bn = tf.layers.batch_normalization(conv,momentum=0.95,epsilon=1e-5,name='bn')


            rl = tf.nn.relu(bn, name='rl')
            resize_bilinear = tf.image.resize_bilinear(rl, size=shape, align_corners=True, name='rb')
        return resize_bilinear

    def pspnet_last(self,input,name,flag=False,reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            if flag:
                deconv_2 = self.deconv2d(inputdata=input, out_channel=128, kernel_size=4,
                                         stride=2, padding='SAME',  use_bias=False, name='deconv_2')
            else:
                deconv_2 = input
            deconv_1 = self.deconv2d(inputdata=deconv_2, out_channel=128, kernel_size=4,
                                                      stride=2, padding='SAME',use_bias=False, name='deconv_1')
            conv1 = tf.layers.conv2d(deconv_1,filters=64,kernel_size=[3,3],strides=[1,1],padding='SAME',use_bias=False,name='conv1')
            bn = tf.layers.batch_normalization(conv1, momentum=0.95, epsilon=1e-5, name='bn')
            conv2 = tf.layers.conv2d(bn,filters=3,kernel_size=[1,1],strides=[1,1],padding='VALID',use_bias=False,name='conv2')
        return conv2
    ...
    def build_autoencoder(self, input_tensor, name, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            shape = tf.shape(relu_6)[1:3]
            print(relu_6.shape,'shape')
            ...
            conv_10_pool1 = self.pspnet_block(relu_10, 60, 90, 60, 90, shape, name='conv_10_pool1')
            conv_10_pool2 = self.pspnet_block(relu_10, 30, 45, 30, 45, shape, name='conv_10_pool2')
            conv_10_pool3 = self.pspnet_block(relu_10, 20, 30, 20, 30, shape, name='conv_10_pool3')
            conv_10_pool4 = self.pspnet_block(relu_10, 10, 15, 10, 15, shape, name='conv_10_pool4')
            print(conv_10_pool1.shape, 'conv_10_pool1')
            print(conv_10_pool2.shape, 'conv_10_pool2')
            print(conv_10_pool3.shape, 'conv_10_pool3')
            print(conv_10_pool4.shape, 'conv_10_pool4')

            # concat_1 = tf.concat([relu_6,conv_6_pool1],axis=-1,name='concat_1')
            # concat_2 = tf.concat([relu_6,conv_6_pool2],axis=-1,name='concat_2')
            concat = tf.concat([relu_6, conv_10_pool4, conv_10_pool3, conv_10_pool2, conv_10_pool1], axis=-1, name='concat')
            print(concat.shape, 'concat')

            output_1 = tf.layers.conv2d(concat, filters=3, kernel_size=[1, 1], strides=[1, 1], padding='VALID',
                                        use_bias=False, name='conv2')
            print(output_1.shape, 'output_1')
            output_2 = self.pspnet_last(concat, name='output_2')
            print(output_2.shape, 'output_2')
            output_3 = self.pspnet_last(concat, name='output_3', flag=True)
            print(output_3.shape, 'output_3')
            #
            ...
            #                             name='skip_output_3')
            # 传统GAN输出层都使用tanh函数激活
            output_3 = tf.nn.tanh(output_3, name='output_3')
    ...
    def compute_autoencoder_loss(self, input_tensor, label_tensor, name, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            output_list = [inference_ret['output_1'], inference_ret['output_2'], inference_ret['output_3']]
        ...
        return loss, inference_ret['output_3']


if __name__ == '__main__':
    input_image = tf.placeholder(dtype=tf.float32, shape=[1, 240, 360, 3])
    auto_label_image = tf.placeholder(dtype=tf.float32, shape=[1, 240, 360, 3])
    rnn_label_image = tf.placeholder(dtype=tf.float32, shape=[1, 240, 360, 1])
