...
class GenerativeNet(cnn_basenet.CNNBaseModel):
    ...
    def pspnet_block(self,input,pool_k_h,pool_k_w,pool_s_h,pool_s_w,shape,name,reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            ...
            # deconv = self.deconv2d(inputdata=rl, out_channel=256, kernel_size=4,
            #                              stride=2, padding='SAME', use_bias=False, name='deconv')
    ...
    def pspnet_last(self,input,name,flag=False,reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            ...
            conv2 = tf.layers.conv2d(bn,filters=3,kernel_size=[1,1],strides=[1,1],padding='VALID',use_bias=True,name='conv2')
        return conv2
    ...
    def build_autoencoder(self, input_tensor, name, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            conv_6_pool1 = self.pspnet_block(relu_6, 60, 90, 60, 90,shape,name='conv_6_pool1')
            conv_6_pool2 = self.pspnet_block(relu_6, 30, 45, 30, 45, shape,name='conv_6_pool2')
            conv_6_pool3 = self.pspnet_block(relu_6, 20, 30, 20, 30, shape,name='conv_6_pool3')
            conv_6_pool4 = self.pspnet_block(relu_6, 10, 15, 10, 15, shape,name='conv_6_pool4')
            print(conv_6_pool1.shape, 'conv_6_pool1')
            print(conv_6_pool2.shape, 'conv_6_pool2')
            print(conv_6_pool3.shape, 'conv_6_pool3')
            print(conv_6_pool4.shape,'conv_6_pool4')

            # concat_1 = tf.concat([relu_6,conv_6_pool1],axis=-1,name='concat_1')
            # concat_2 = tf.concat([relu_6,conv_6_pool2],axis=-1,name='concat_2')
            concat = tf.concat([relu_6,conv_6_pool4,conv_6_pool3,conv_6_pool2,conv_6_pool1],axis=-1,name='concat')
            print(concat.shape,'concat')

            output_1 = tf.layers.conv2d(concat,filters=3,kernel_size=[1,1],strides=[1,1],padding='VALID',use_bias=True,name='conv2')
